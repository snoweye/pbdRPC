...
\item \code{.pbd_env$RPC.CT} is main RPC controls taking care several basic
      ...
      \code{plink()}.
\item \code{.pbd_env$RPC.LI} has information of login account for logining to
...
\end{Command}
to access the server and ask it to execute a command \code{whoami}.
...
from a termial \code{cmd.exe}. See Section~\ref{sec:basic_plink} for details.
...
In Windows system under \code{cmd.exe}, one may similarly use next
