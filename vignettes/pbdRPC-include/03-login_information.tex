...
Also, \code{ssh} uses ``\code{-p}'' to input the server port argument.
\code{plink.exe} uses ``\code{-P}'' to input the server port argument.
...
By default, the \code{prive.key.ppk} will read the file from the current
